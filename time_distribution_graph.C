...
// Somma di gaussiane
double Gaussian_sum(double x, const std::vector<double>& times, double tpe) {
    double G = 0;
    for (double t : times) {
        double gaussian_value = gaussian(x, t, GAUSSIAN_STDDEV);
        double exponential_part = exp(-(x*tpe));
...
void processFile(const std::string& filename, std::vector<double>& N, std::vector<double>& tot_l, std::vector<double>& tot_h,int graph) {
    ...
    for (int i = 0; i < NUM_POINTS; ++i) {
      G[i] = Gaussian_sum(x[i], times,0.6);
	...
	expectedGraph->SetLineWidth(2);
      ...
      std::string outputFilename = "TTS_0.8_exp0.6" + filename + ".png";
